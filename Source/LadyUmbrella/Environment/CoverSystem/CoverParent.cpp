...
ACoverParent::ACoverParent()
{
	...
	if (IsValid(BillboardRoot))
	{
		...
		if (IsValid(Billboard))
		{
			...
			static ConstructorHelpers::FObjectFinder<UTexture2D> ShieldSpriteRef (TEXT("/Game/Textures/Sprites/Icons/CoverSystem/T_ShieldIcon.T_ShieldIcon"));
			if (ShieldSpriteRef.Object != nullptr)
			{
				Billboard->SetSprite(ShieldSpriteRef.Object);
			}
		}
