...
bool UCoverSpline::TestTallCoverAtTime(const float Time) const
{
	...
	QueryParams))
	{
		...
		{
			if (Hit.GetActor() == TryGetParent() || Hit.GetActor()->ActorHasTag(ENVIRONMENT_COVER_TAG))
			{
				return true;
			}
		}
...
void UCoverSpline::DestroyCover()
{
	...
	for (int i = CharactersUsingTheSpline.Num() - 1; i >= 0; i--)
	{
		...
		{
			CharactersUsingTheSpline[i]->NotifyExitCover();
		}
	}
...
bool UCoverSpline::IsPointReachableForward(bool FromPoint0, AAICoverEntryPoint* Point) const
{
	...
	if (abs(FVector::DotProduct(FindForwardRotationAtTime(OriginSection).Vector(), OtherForwardVector)) > 0.05f)
	{
		return false;
	}
	...
	// If the other point is not in the direction our Spline goes into.
	if (FVector::DotProduct(ContinuousVector, OtherForwardVector) > -0.98f)
	{
		return false;
	}
